...
class FavoriteRepo {
  static Future<FavoriteResponse> getWishList() async {
    try {
      final Response response = await locator<ApiService>().dioClient.get(
            '${locator<ApiService>().dioClient.options.baseUrl}wishlist',
          );
      ...
      if (error.response?.statusCode == 401 ||
        ...
        return FavoriteResponse.fromJsonError(
            json: error.response?.data, error: error);
      } else if (error.error is SocketException) {
        print('xXx xc ${error.error}');
        return FavoriteResponse.makeError(
            error: error,
