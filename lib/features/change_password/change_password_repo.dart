...
class ChangePasswordPostRepo {
  Future<ChangePasswordResponse> changePasswordPost(ChangePasswordRequest request) async {
    FormData formData = FormData.fromMap(request.toJson());
    print(formData.fields);
    try {
      final Response response = await locator<ApiService>().dioClient.post(
            '${locator<ApiService>().dioClient.options.baseUrl}change-password',
            data: formData,
          );
      ...
      if (error.response?.statusCode == 401 ||
        ...
        return ChangePasswordResponse.fromJsonError(
            json: error.response?.data, error: error);
      } else if (error.error is SocketException) {
        print('xXx xc ${error.error}');
        return ChangePasswordResponse.makeError(
