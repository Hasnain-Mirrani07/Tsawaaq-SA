...
class MyAddressesRepo {
  static Future<MyAddressesResponse> getAddresses() async {
    try {
      final Response response = await locator<ApiService>().dioClient.get(
        '${locator<ApiService>().dioClient.options.baseUrl}addresses',
      );

      return MyAddressesResponse.fromJson(response.data);
    } on DioError catch (error) {
      if (error.response?.statusCode == 401 ||
          error.response?.statusCode == 422) {
        return MyAddressesResponse.fromJsonError(
            json: error.response?.data, error: error);
      } else if (error.error is SocketException) {
        print('xXx xc ${error.error}');
        return MyAddressesResponse.makeError(
        ...
        return MyAddressesResponse.makeError(error: error, errorMsg: errorDescription);
      }
    }
  }
}
