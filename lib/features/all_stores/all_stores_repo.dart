...
class StoresRepo {
  static Future<StoresResponse> getStores({int id = 0}) async {
    try {
      final Response response = await locator<ApiService>().dioClient.get(
            '${locator<ApiService>().dioClient.options.baseUrl}stores?category_id=$id',
          );
      ...
      if (error.response?.statusCode == 401 ||
        ...
        return StoresResponse.fromJsonError(
            json: error.response?.data, error: error);
      } else if (error.error is SocketException) {
        print('xXx xc ${error.error}');
        return StoresResponse.makeError(
